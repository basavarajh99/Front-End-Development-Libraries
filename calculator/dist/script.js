...
React.createElement("span", { className: "input", id: "display" }, input));
...
const App = () => {
  ...
  const handleOperators = value => {
    ...
    if (calculatorData.length) {
      ...
      const beforeLastChar =
      calculatorData.charAt(calculatorData.length - 2);

      const beforeLastCharIsOperator =
      operators.includes(beforeLastChar) || beforeLastChar === "*";

      const lastChar =
      calculatorData.charAt(calculatorData.length - 1);
